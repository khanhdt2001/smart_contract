    ...
    describe("lenderMakeOffer", () => {
        it("fail", async () => {
            await expect(
                lendingFactory
                    .connect(address2)
                    .lenderMakeOffer(1, 10, 12, 1814400, 3)
            ).to.be.revertedWith("Address must be registered");
        });
        it("fail", async () => {
        ...
        it("fail", async () => {
            ...
            await expect(
                lendingFactory
                    .connect(address2)
                    .lenderMakeOffer(0, 10, 12, 1814400, 4)
            ).to.be.revertedWith(
        ...
        it("fail", async () => {
         ...
         await expect(
             lendingFactory
                 .connect(address2)
                 .lenderMakeOffer(0, 10, 12, 3456000, 1)
         ).to.be.revertedWith(
             "Lending: offer time and payment time are not valid"
         );
     });
        it("success", async () => {
            ...
            const res = await lendingFactory
                .connect(address2)
                .lenderMakeOffer(0, 10, 12, 1814400, 3);
            await expect(res)
                .to.be.emit(lendingFactory, "LenderMakeOffer")
                .withArgs(address2.address, 0, 0, 10, 12, 1814400, 3);
        });
        it("success", async () => {
            await setUpLenderMakeOffer();
            await lendingFactory
            ...
            await expect(res)
                .to.be.emit(lendingFactory, "LenderMakeOffer")
                .withArgs(address2.address, 0, 1, 10, 12, 1814400, 3);
        });
    });
